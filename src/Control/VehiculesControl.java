...
public class VehiculesControl {
    ...
    public Vehicules searchVehicule(String dniVehicule){
        ResultSet resultSet = Desktop.db.search("SELECT * FROM `vehicules` where DNI='"+dniVehicule+"'");
        if(resultSet!= null){
            try{
                resultSet.next();
    ...
    public boolean deleteVehicule(Vehicules vehicule){
        if(vehiculesList().size() >=2){
            return Desktop.db.delete("delete from vehicules where DNI='"+vehicule.getVehiculeDNI()+"'");
        }
        return false;
    }
    ...
    public ArrayList filterVehiculesListByDNI(String filter){
        ArrayList<Vehicules> vehicules = new ArrayList<>();
        ResultSet resultSet = Desktop.db.list("select * from vehicules where DNI like '%"+filter+"%'");
        if(resultSet != null){
            try{
                while(resultSet.next()){
                    vehicules.add(new Vehicules(resultSet.getString(2),
