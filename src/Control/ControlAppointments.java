...
//Fitro de citas
    ...
    public ArrayList appoinmentFilter(java.sql.Date date){
        ArrayList<Appointment> appoinment = new ArrayList<>();
        
        ResultSet resultSet = Desktop.db.list("SELECT * FROM `appointments` where Date='"+date+"'");
        if(resultSet != null){
            try{
                while(resultSet.next()){
                    appoinment.add(new Appointment(
                        resultSet.getDate(2),
                        resultSet.getTime(3),
                        new Vehicules(
                                resultSet.getString(4),
